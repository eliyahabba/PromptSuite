...
def main(template, data, output, format, max_variations_per_row, variations_per_field, api_key):
    ...
    try:
        ...
        ps = PromptSuiteEngine(max_variations_per_row=effective_max_variations_per_row)

        # Generate variations
        click.echo("Generating variations...")
        variations = ps.generate_variations(
        ...
        # Save output
        ps.save_variations(variations, output, format=format)
        click.echo(f"Saved to {output}")

        # Show statistics
        stats = ps.get_stats(variations)
        click.echo("\nStatistics:")
